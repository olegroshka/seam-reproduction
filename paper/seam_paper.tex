...
This is the "no cross-block coupling" baseline. It supplies the null hypothesis tested in \S\ref{sec:test} (Theorem~\ref{thm:T5}). The converse holds unconditionally under (A1)--(A3): the full LOC Schur complement $\Mloc := (\Sigmaf)_\LOC - (\Sigmaf)_{\LOC, \REM} (\Sigmaf)_\REM^{-1} (\Sigmaf)_{\REM, \LOC}$ is positive definite (since $\Sigmaf \succ 0$ by (A1) and (A2), the partitioned-inverse identity gives $\Mloc^{-1} = (\Sigmaf^{-1})_{\LOC, \LOC} \succ 0$), and the bilinear form $\sigma^2_C = \Ntot^{-1} \trace[A_{\REM, \LOC} \Mloc A_{\REM, \LOC}^\top]$ vanishes iff $A_{\REM, \LOC} = \mathbf{0}$. No further genericity assumption on the diagonal Schur blocks is needed.
...
\paragraph{Remark on the converse.} Under (A1)--(A3), the full LOC Schur complement $\Mloc$ is positive definite ($\Sigmaf \succ 0$ from stability and (A2) yields $\Mloc^{-1} = (\Sigmaf^{-1})_{\LOC, \LOC} \succ 0$ by the partitioned-inverse identity). The full bilinear representation
\[
\sigC = \frac{1}{\Ntot} \trace[A_{\REM, \LOC} \Mloc A_{\REM, \LOC}^\top]
\]
then yields $\sigC = 0 \iff A_{\REM, \LOC} = \mathbf{0}$ unconditionally. The diagonal-blocks-PD condition $M(\LOC_j, \LOC_j) \succ 0$ for each $j$ is implied by $\Mloc \succ 0$ but is not sufficient on its own --- it would still permit off-diagonal cross-term cancellation in $\sigC$. The load-bearing condition is $\Mloc \succ 0$, automatic under (A1)--(A3).
