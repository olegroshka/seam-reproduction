...
% ─── Title ───────────────────────────────────────────────────────────────────
\title{Asymptotic variance reduction at remainder blocks under joint VAR(1) over per-block AR(1)\thanks{Reproduction code: \url{https://anonymous.4open.science/r/seam-reproduction}.}}
...
% ─── Abstract ────────────────────────────────────────────────────────────────
\begin{abstract}
For stationary VAR(1) processes with a LOCAL/REMAINDER block partition, we derive a closed-form expression $\sigma^2_C$ for the asymptotic forecast-error variance reduction at REMAINDER under full-state joint-VAR conditioning relative to within-REMAINDER AR(1) BLP conditioning (Theorem~\ref{thm:T1}). The expression is the trace of a Schur complement of $(\Sigma_f)_\REM$ in a $2 \times 2$ block matrix of $A$- and $\Sigma_f$-derived quantities, and specializes the standard partitioned-VAR forecast-error covariance machinery \cite[Ch.~2.2.2--3.5]{Lutkepohl2005} to this within-block-AR(1) baseline. We give a cluster decomposition of $\sigma^2_C$ along the LOCAL partition with a structural vanishing condition that, under joint Gaussianity, corresponds to graphical-model conditional independence in the sense of Lauritzen \cite{Lauritzen1996} (Theorem~\ref{thm:T2}, Corollary~\ref{cor:cond-indep}). The sampling theory --- $\sqrt{T}$-consistency (Theorem~\ref{thm:T3}), asymptotic normality via delta method with closed-form variance $V$ (Theorem~\ref{thm:T4}), finite-sample bias (Proposition~\ref{prop:P6}), and a Wald test for cross-block coupling coinciding with the partition-specialized Granger non-causality test (Theorem~\ref{thm:T5}) --- follows from standard OLS-VAR machinery applied to the smooth functional $\varphi(A, \Sigma_f)$. Five limiting regimes admit closed-form simplifications or bounds (Propositions~\ref{prop:P1}--\ref{prop:P5}). Near the boundary of (A3), $V \sim 1/\lambda_{\min}((\Sigma_f)_\REM)^4$, which is more severe than standard multicollinearity scaling. The natural empirical setting is heterogeneous panel forecasting with a given block partition; companion empirical work appears separately.
\end{abstract}
...
Many multivariate time series exhibit a partitioned structure where some components are tightly coupled within identifiable blocks and others form a heterogeneous remainder. In heterogeneous economic panel forecasting --- where actors group by sector, region, or latent factor structure --- the practitioner faces a methodological choice between joint-VAR architectures that model cross-block coupling globally and block-diagonal architectures that fit per-block dynamics in isolation. The question we address is: under stationary VAR(1) dynamics, what is the asymptotic forecast-error variance gap between these two architectures at REMAINDER blocks, and what does it depend on?

We provide a closed-form answer to that question. The asymptotic forecast-variance reduction at REMAINDER blocks from joint-VAR conditioning on $f_{t-1}$ versus within-REMAINDER AR(1) BLP conditioning on $f_{\REM, t-1}$ admits a closed-form expression $\sigma^2_C$ depending on $(A, \Sigma_f)$ alone (Theorem~\ref{thm:T1}). The closed form is the trace of a Schur complement of $(\Sigma_f)_\REM$ in a $2 \times 2$ block matrix involving $A$, and is a specialization of the standard partitioned-VAR forecast-error covariance machinery \cite[Ch.~2.2.2--3.5]{Lutkepohl2005} to this particular within-block-AR(1) baseline. Theorem~\ref{thm:T2} gives a cluster decomposition of $\sigma^2_C$ along the LOCAL partition into pairwise LOCAL $\leftrightarrow$ REMAINDER coupling terms whose off-diagonal cross-terms vanish (structurally, for all admissible $A_{\REM, \LOC}$) when REMAINDER separates the LOCAL blocks in the conditional-independence graph of the joint-Gaussian factor distribution --- a connection to partial-correlation graphical models following Lauritzen \cite{Lauritzen1996}.

The natural empirical setting is heterogeneous panel forecasting with a given block partition --- e.g., country / sector / region groupings in macroeconomic panel data --- where the partition is supplied by domain knowledge and $\sigma^2_C$ quantifies the partition-level forecast-variance gap from the joint-vs-block-diagonal architecture choice. The cluster decomposition (Theorem~\ref{thm:T2}) further admits application to network time series with community structure, where the partition is given by community detection and $\sigma^2_C$ measures within-vs-cross-community forecast information. Other potential domains (spatial sensor networks, functional brain imaging, hierarchical forecasting) share the partitioned-state structure but typically require lag $p \ge 2$, which is outside the present VAR(1) scope; companion empirical work on macroeconomic panels appears separately.

\paragraph{Contributions.}
\begin{enumerate}[label=(\roman*),leftmargin=1.5em,itemsep=0.1em]
\item A closed-form expression $\sigma^2_C$ for the asymptotic REMAINDER variance reduction (Theorem~\ref{thm:T1}), specializing standard partitioned-VAR forecast-error covariance algebra to the within-REMAINDER-AR(1)-BLP baseline.
\item A cluster decomposition of $\sigma^2_C$ along the LOCAL partition (Theorem~\ref{thm:T2}) with a structural vanishing condition that, under joint Gaussianity, instantiates the Lauritzen \cite{Lauritzen1996} Schur-complement / conditional-independence equivalence at the LOCAL partition (Corollary~\ref{cor:cond-indep}).
\item A decomposition of the one-step state-propagation variance: $\sigma^2_{\mathrm{init}} = \sigma^2_C + \sigma^2_{\mathrm{within\text{-}REM\text{-}AR(1)\text{-}BLP}}$ (Corollary~\ref{cor:sigma-init}).
\item Closed-form simplifications or bounds under five limiting regimes (Propositions~\ref{prop:P1}--\ref{prop:P5}).
\item Sampling theory via standard OLS-VAR + delta-method machinery applied to the smooth functional $\varphi(A, \Sigma_f)$: consistency (Theorem~\ref{thm:T3}), asymptotic normality with $V = (\nabla \varphi)^\top \Omega (\nabla \varphi)$ (Theorem~\ref{thm:T4}), finite-sample bias (Proposition~\ref{prop:P6}), and a near-degeneracy result $V \sim 1/\lambda_{\min}((\Sigmaf)_\REM)^4$ that is more severe than standard multicollinearity scaling (\S\ref{sec:near-deg}).
\item A Wald test for cross-block coupling (Theorem~\ref{thm:T5}); equivalently, the standard Granger non-causality test specialized to the LOCAL/REMAINDER partition with closed-form inverse covariance $\Mloc \otimes (\Sigmae)_\REM^{-1}$.
\item Symbolic and numerical verification (Sympy + Monte Carlo) of all closed-form claims at machine precision, with a pre-registered simulation suite covering Theorems~\ref{thm:T1}--\ref{thm:T5} and Propositions~\ref{prop:P2}--\ref{prop:P6} (Appendix~\ref{app:H}).
\end{enumerate}
...
The two predictors compared by $\sigma^2_C$ --- joint conditioning on $f_{t-1}$ versus restricted conditioning on $f_{\REM, t-1}$ --- are precisely the predictors underlying the standard Granger-causality framework \cite[Ch.~2.3, Ch.~7]{Lutkepohl2005}: $\sigma^2_C > 0$ if and only if the LOCAL components Granger-cause REMAINDER, and Theorem~\ref{thm:T5}'s Wald test specializes the standard Granger non-causality test to the LOCAL/REMAINDER partition. Geweke \cite{Geweke1982} introduced a quantitative measure of Granger-causality strength via a log-determinant ratio of prediction-error variances; $\sigma^2_C$ is in the same conceptual family but uses a trace-difference (mean-squared-error scale) rather than entropy scale, and admits a closed-form Schur-complement expression at the LOCAL/REMAINDER partition with within-block AR(1) restriction --- a setting Geweke's framework does not specialize to. Chudik and Pesaran \cite{ChudikPesaran2015} address heterogeneous panel forecasting with weakly exogenous regressors; the present $\sigma^2_C$ result complements this literature by characterizing the partition-level forecast-variance gap directly. The specific combination --- closed-form $\sigma^2_C$, cluster-decomposed along the LOCAL partition, with the within-REMAINDER-AR(1)-BLP baseline --- does not appear in the prior literature to our knowledge.

\paragraph{Paper structure.} \S\ref{sec:setup} establishes notation. \S\ref{sec:main} states Theorem~\ref{thm:T1}, the cluster decomposition (Theorem~\ref{thm:T2}), and the structural corollaries. \S\ref{sec:varp} extends the framework to VAR$(p)$ (Theorems~\ref{thm:T1p}--\ref{thm:T5p}). \S\ref{sec:sampling} develops the sampling theory for VAR$(1)$ (Theorems~\ref{thm:T3}--\ref{thm:T5} and Proposition~\ref{prop:P6}). \S\ref{sec:sim} verifies via simulation. \S\ref{sec:disc} discusses connections and open questions. Five named limiting regimes for VAR$(1)$ (Propositions~\ref{prop:P1}--\ref{prop:P5}) appear in Appendix~\ref{app:C}; VAR$(p)$ derivations and numerical-verification tables in Appendix~\ref{app:VARp}; other proofs in Appendices~\ref{app:A}--\ref{app:G}; full simulation methodology in Appendix~\ref{app:H}.
...
\end{equation}
The right-hand side is a deterministic function of $(A, \Sigma_f)$ --- equivalently, of $(A, \Sigma_\varepsilon)$ via (\ref{eq:Lyap}) --- and the chosen partition; it contains no tuning parameters or hyperparameters beyond the model primitives, which themselves carry $K^2 + K(K+1)/2$ parameters that must be estimated from data in the plug-in version $\hatsigC$ studied in \S\ref{sec:sampling}.
\end{theorem}
...
\begin{corollary}[Cross-term vanishing condition]\label{cor:cond-indep}
If $M(\LOC_j, \LOC_k) = \mathbf{0}$ for all $j \ne k$, then $\sigma^2_{C, \mathrm{cross}} = 0$. Conversely, $\sigma^2_{C, \mathrm{cross}} = 0$ for all admissible $A_{\REM, \LOC}$ if and only if $M(\LOC_j, \LOC_k) = \mathbf{0}$ for all $j \ne k$; for a fixed $A$, $\sigma^2_{C, \mathrm{cross}} = 0$ can also arise from cancellation across $(j, k)$ pairs without $M(\LOC_j, \LOC_k) = \mathbf{0}$. Under joint Gaussianity, $M(\LOC_j, \LOC_k) = \mathbf{0}$ is equivalent to $f_{\LOC_j} \perp f_{\LOC_k} \mid f_\REM$ --- REMAINDER separates the LOCAL blocks in the conditional-independence graph.
\end{corollary}
...
\subsection{Decomposition of the one-step state-propagation variance}\label{sec:cor-init}

Define $\sigma^2_{\mathrm{init}} := \Ntot^{-1} \trace[(A \Sigma_f A^\top)_\REM]$, the asymptotic uncertainty of $f_{\REM, t}$ explained by the joint stationary state's first-step propagation.

\begin{corollary}[$\sigma^2_{\mathrm{init}}$ decomposition]\label{cor:sigma-init}
Under (A1)--(A3),
\begin{equation*}
...
% ════════════════════════════════════════════════════════════════════════════
\section{Extension to VAR$(p)$}\label{sec:varp}
% ════════════════════════════════════════════════════════════════════════════

The closed-form identity, the cluster decomposition, and the sampling theory all lift to VAR$(p)$ for $p \ge 2$ via the companion-form embedding. The graphical-model conditional-independence interpretation, however, becomes a strictly richer condition. Derivations and numerical verification are in Appendix~\ref{app:VARp}.

\paragraph{Setup.} Let $f_t = \sum_{h=1}^{p} A_h f_{t-h} + \varepsilon_t$ be a stable $K$-dim VAR$(p)$ with $\varepsilon_t \stackrel{iid}{\sim} (\mathbf{0}, \Sigmae)$ and companion-form transition $\widetilde A \in \mathbb{R}^{Kp \times Kp}$ of spectral radius $< 1$. Write $R(h) := \E[f_t f_{t-h}^\top]$ (so $R(0) = \Sigmaf$), and define the $K_\REM p \times K_\REM p$ block-Toeplitz REMAINDER autocovariance $\Gamma$ with $(i, j)$-block $(R(|i-j|))_\REM$ if $j \ge i$ and $(R(i-j))_\REM^\top$ otherwise, alongside $C := [(R(1))_\REM, \ldots, (R(p))_\REM] \in \mathbb{R}^{K_\REM \times K_\REM p}$. The joint VAR$(p)$ predictor of $f_{\REM, t}$ has forecast-error covariance $(\Sigmae)_\REM$; the restricted predictor --- best-linear-predictor on $(f_{\REM, t-1}, \ldots, f_{\REM, t-p})$ --- has forecast-error covariance $(\Sigmaf)_\REM - C \Gamma^{-1} C^\top$. The estimand $\sigma^{2,(p)}_C$ is defined as in (\ref{eq:estimand}).

\begin{theorem}[Closed form, VAR$(p)$]\label{thm:T1p}
Under (A1)--(A3) extended to VAR$(p)$,
\begin{equation}\label{eq:T1p}
\sigma^2_C{}^{(p)} = \frac{1}{\Ntot} \trace\!\left[\sum_{h=1}^{p} (A_h R(h)^\top)_\REM - C\,\Gamma^{-1}\,C^\top\right].
\end{equation}
For $p = 1$, $C = (A\Sigmaf)_\REM$ and $\Gamma = (\Sigmaf)_\REM$, recovering Theorem~\ref{thm:T1}.
\end{theorem}

The proof follows the four-step argument of Theorem~\ref{thm:T1} verbatim, with the VAR$(p)$ Yule-Walker block identity $(\Sigmae)_\REM = (\Sigmaf)_\REM - \sum_h (A_h R(h)^\top)_\REM$ replacing the single-step Lyapunov substitution. The right-hand side is the trace of the Schur complement of $\Gamma$ in the $2 \times 2$ block matrix involving $C$, $C^\top$, and $\sum_h (A_h R(h)^\top)_\REM$ under the joint VAR$(p)$ stationary law.

\paragraph{Companion-form representation.} $\sigma^{2,(p)}_C$ admits an equivalent expression as the trace of Theorem~\ref{thm:T1}'s Schur complement applied at the companion VAR$(1)$ in the $Kp$-dimensional state $F_t = (f_t, f_{t-1}, \ldots, f_{t-p+1})$ with the lifted partition $\REM_\text{comp} := \{(\REM, t-h) : h = 0, \ldots, p-1\}$. The companion-form innovation covariance is rank-deficient ($B \Sigmae B^\top$ with $B = (I_K, \mathbf{0}, \ldots, \mathbf{0})^\top$, violating (A2) at the lifted system), but the Schur form requires only $(\Sigma_F)_{\REM_\text{comp}} \succ 0$ --- equal to $\Gamma$, automatic under (A1)+(A2). Both representations agree to machine precision in all tested regimes (Appendix~\ref{app:VARp}, Table~\ref{tab:varp_E_summary}).

\begin{theorem}[Cluster decomposition, VAR$(p)$]\label{thm:T2p}
With $\widetilde M^\text{comp}(a, b) := (\Sigma_F)_{a,b} - (\Sigma_F)_{a, \REM_\text{comp}} (\Sigma_F)_{\REM_\text{comp}}^{-1} (\Sigma_F)_{\REM_\text{comp}, b}$,
\begin{equation}\label{eq:T2p}
\sigma^2_C{}^{(p)} = \sum_{j=1}^{J} \sigma^2_{C, \LOC_{j,\text{comp}} \leftrightarrow \REM_\text{comp}} + \sigma^{2,\text{comp}}_{C, \mathrm{cross}},
\end{equation}
where the per-LOC diagonal and cross terms are bilinear traces in $\widetilde M^\text{comp}$ blocks against the $(\REM_\text{comp}, \LOC_{j,\text{comp}})$ sub-blocks of $\widetilde A$ (formulas in Appendix~\ref{app:VARp}).
\end{theorem}

\begin{corollary}[Cross-term vanishing, VAR$(p)$]\label{cor:cond-indep-p}
$\sigma^{2,\text{comp}}_{C, \mathrm{cross}} = 0$ for all admissible $\widetilde A$ iff $\widetilde M^\text{comp}(\LOC_{j,\text{comp}}, \LOC_{k,\text{comp}}) = \mathbf{0}$ for every $j \ne k$. Under joint Gaussianity, this is REMAINDER-comp separating LOCAL-comp blocks in the $(Kp)$-dimensional companion-state conditional-independence graph of $F_t$.
\end{corollary}

\begin{remark}[Contemporaneous CI is insufficient for $p \ge 2$]\label{rem:contemp-CI-fail}
The VAR$(1)$ Corollary~\ref{cor:cond-indep} statement --- ``$\sigma^2_{C, \mathrm{cross}}$ vanishes structurally iff REM separates LOC blocks in the contemporaneous $\Sigmaf$ conditional-independence graph'' --- does \emph{not} extend to $p \ge 2$. Finite $p$-lag conditioning on REM leaks dependence through unobserved past REM history. Even DGPs engineered with strong structural restrictions (no LOC--LOC coupling at any lag, block-diagonal $\Sigmae$, AR$(1)$-within-REM dynamics, REM appearing only at lag~1 in all blocks) generically retain a non-zero $\sigma^{2,\text{comp}}_{C, \mathrm{cross}}$ of order $1$--$10\%$ of $\sigma^{2,(p)}_C$. The $(Kp)$-dim CI graph is the load-bearing object; the contemporaneous $K$-dim CI graph is a VAR$(1)$-specific simplification. Empirical evidence in Appendix~\ref{app:VARp}.
\end{remark}

\paragraph{Sampling theory.} The plug-in $\widehat\sigma^{2,(p)}_C := \varphi^{(p)}(\widehat A_{1,T}, \ldots, \widehat A_{p,T}, \widehat\Sigmaf{}_{,T})$ is $\sqrt{T}$-consistent and asymptotically normal by the delta-method argument of Theorems~\ref{thm:T3}--\ref{thm:T4}, applied to the smooth functional $\varphi^{(p)}$ on the OLS-VAR$(p)$ parameter manifold. The asymptotic variance $V^{(p)} = (\nabla\varphi^{(p)})^\top \Omega^{(p)} (\nabla\varphi^{(p)})$ uses the joint asymptotic covariance of $(\mathrm{vec}\,\widehat A_1, \ldots, \mathrm{vec}\,\widehat A_p, \vech\widehat\Sigmaf)$ from \cite[Prop.~3.2, Ch.~3.4]{Lutkepohl2005}. Near $\partial\{\Gamma \succ 0\}$, $V^{(p)} \sim 1/\lambda_{\min}(\Gamma)^4$ --- the same exponent as VAR$(1)$, with $\Gamma$ replacing $(\Sigmaf)_\REM$. The finite-sample bias retains the leading $b/T + o(T^{-1})$ form.

\begin{theorem}[Wald test, VAR$(p)$]\label{thm:T5p}
Stack $\eta_T := \mathrm{vec}(\widehat A_{1, \REM, \LOC, T}, \ldots, \widehat A_{p, \REM, \LOC, T}) \in \mathbb{R}^{p K_\REM K_\LOC}$. Under $H_0: A_{h, \REM, \LOC_j} = \mathbf{0}$ for all $h, j$,
\begin{equation}\label{eq:T5p}
W_T^{(p)} := T \cdot \eta_T^\top \!\left[\widehat M_\LOC^{(p)} \otimes \widehat{(\Sigmae)}_\REM^{-1}\right] \eta_T \xrightarrow{d} \chi^2(p K_\REM K_\LOC),
\end{equation}
where $\widehat M_\LOC^{(p)} = ((\widehat\Gamma_p^{-1})_{\LOC \times \text{all lags},\, \LOC \times \text{all lags}})^{-1}$ is the Schur complement of $(\widehat\Gamma_p)_\REM$ at the LOC sub-block of the OLS-regressor second-moment matrix $\widehat\Gamma_p$. For $p = 1$, $\widehat M_\LOC^{(p)} = \widehat\Mloc$ and the test reduces to Theorem~\ref{thm:T5}.
\end{theorem}

\paragraph{Numerical verification.} Theorem~\ref{thm:T1p} is verified to machine precision across $p \in \{2, 3, 5, 7, 10\}$ and $K \in \{4, 6, 8, 12\}$ (25/25 cells, max residual $9 \times 10^{-15}$), and certified-exact via Sympy rational arithmetic at $K=3, p=2$ (Appendix~\ref{app:VARp_tables}). Companion-form representation matches the direct closed form to $10^{-14}$ across the same grid. Sampling theory at $p=2$, $K=6$: $\sqrt{T}$-rate consistency with $\sqrt{T} \cdot \mathrm{SD}$ ratio $1.135$; CI$95$ coverage $96.0\%$ at $T = 2000$. Wald test (Theorem~\ref{thm:T5p}) empirical size: $0.0525$ at $T=1000$, $0.0450$ at $T=2000$ (nominal $0.05$). Near-degeneracy log-log slope $-4.0000$. Closed form computes in sub-millisecond at $K=30$. Reproduction scripts: \path{verify/var_p/}.

\paragraph{Limiting cases (VAR$(1)$).} Five named structural regimes of the VAR$(1)$ baseline (block-diagonal $A$, rank-one cross-coupling, symmetric $A$ with isotropic $\Sigmae$, block-equicorrelated REMAINDER, sparse-$A$ bound) admit closed-form simplifications or bounds; statements, discussion, and proofs appear in Appendix~\ref{app:C}. The analogous specializations for VAR$(p)$ are left to future work.
...
\end{equation*}
Plug-in computable via $\widehat{\tau} = \widehat{V} \cdot \widehat{\lambda}_{\min}^4 / (\varepsilon^2 (\widehat{\sigma}^2_C)^2)$. The $1/\lambda_{\min}^4$ exponent arises from the bilinear $(\Sigma_f)_\REM^{-1}$ dependence in (\ref{eq:T1}), giving two inverse-factor contributions to $\partial \varphi / \partial \Sigma_f$ rather than the single inverse factor that yields the $1/\lambda_{\min}^2$ scaling familiar from OLS multicollinearity. When $(\Sigma_f)_\REM$ is near-singular, the required $T$ for a given relative-SE target therefore grows as $\lambda_{\min}^{-4}$ rather than $\lambda_{\min}^{-2}$; whether this is binding for a given dataset is checked via $\widehat\tau$ directly. A small ridge regularisation on $(\widehat{\Sigma}_f)_\REM$ can be used at small $T$. Detailed derivation in Appendix~\ref{app:F}.
...
An alternative test could be based directly on $\widehat{\sigma}^2_C$. Under $H_0: A_{\REM, \LOC} = \mathbf{0}$ (equivalent to $\sigma^2_C = 0$ under (A1)--(A3) by Proposition~\ref{prop:P1} and its converse), the parameter lies at a critical point of $\varphi$: by first-order conditions for a smooth functional at its global minimum (Proposition~\ref{prop:P1} plus Schur-complement non-negativity), $\nabla \varphi |_{H_0} = \mathbf{0}$ identically. Hence $V|_{H_0} = 0$ and the standard $\sqrt{T}$-normal limit of Theorem~\ref{thm:T4} degenerates. The boundary-asymptotic limit (Appendix~\ref{app:G-bdry}) is
\[
T \cdot \widehat{\sigma}^2_C \to_d \tfrac{1}{2} Z^\top H_0 Z, \qquad Z \sim \mathcal{N}(\mathbf{0}, \Omega),
\]
a weighted sum of independent $\chi^2_1$ random variables with weights given by the eigenvalues of $\Omega^{1/2} H_0 \Omega^{1/2}$; tail probabilities via Imhof \cite{Imhof1961} or moment-matching \cite{Davies1980}. The bias-corrected estimator $\widehat{\sigma}^2_{C, \mathrm{bc}}$ shifts the limit by $-\tfrac{1}{2}\trace(H_0 \Omega)$, centring the limiting quadratic form to mean zero. A power calculation under local alternatives shows the boundary-asymptotic test on $\widehat{\sigma}^2_C$ is dominated pointwise by the Wald test of Theorem~\ref{thm:T5} by a factor of at least $\Ntot / \lambda_{\max}((\Sigma_\varepsilon)_\REM)$ in non-centrality \cite{Andrews2001,SelfLiang1987}. We therefore present the Wald test as the primary tool for cross-block coupling testing.
\end{remark}
...
A pre-registered simulation suite (Appendix~\ref{app:H}, designs D1--D7) verifies the closed-form identities and asymptotic theory across $K \in \{8, 12\}$, partition shapes ranging $(4, 4)$ to $(12, 4)$, $\sigma_{\mathrm{cross}} \in [0, 0.8]$, and $T \in \{50, \ldots, 20000\}$. Headline results: Theorem~\ref{thm:T1} closed form matches empirical MC to within $0.5\%$ at $\sigma^2_C > 0.1$ across six block-size ratios (max $3.08\%$); $\widehat{\sigma}^2_C$ achieves $0.19\%$ rel.\ err.\ at $T = 5000$ with $\sqrt{T}\cdot\mathrm{SD}$ max/min ratio $1.107 < 1.15$ (Theorem~\ref{thm:T3}, $\sqrt{T}$-rate); delta-method CI coverage at $T = 1000$ is $94.3\%$ at nominal $0.95$ (Theorem~\ref{thm:T4}); Propositions~\ref{prop:P2}--\ref{prop:P4} closed forms agree at $\le 3\%$ rel.\ err.; the near-degeneracy V3.a parametric verification of $V \sim 1/\lambda_{\min}^4$ gives $d_V = 4.0029$ with $R^2 = 1.0000$. The Wald test (Theorem~\ref{thm:T5}) reaches nominal size at $T \ge 1000$ (empirical $\alpha = 0.058$ at nominal $0.05$, $K = 12$) and unit power for $\sigma^2_C \ge 1.4 \times 10^{-2}$. Full DGP specifications, per-design tables, and discussion of the Lyapunov-inconsistent verification path for D6 appear in Appendix~\ref{app:H}; VAR$(p)$ verification results (E1--E12) appear in Appendix~\ref{app:VARp}.

% ════════════════════════════════════════════════════════════════════════════
\section{Discussion}\label{sec:disc}
% ════════════════════════════════════════════════════════════════════════════

\paragraph{Connections to related theoretical literature.} The cluster-decomposition geometry (Theorem~\ref{thm:T2}, Corollary~\ref{cor:cond-indep}) connects directly to several theoretical literatures. The Schur-complement-iff-conditional-independence identity for joint-Gaussian distributions \cite{Lauritzen1996} is the structural anchor: $\sigma^2_{C, \mathrm{cross}}$ vanishes structurally (for all admissible $A_{\REM, \LOC}$) iff REMAINDER separates the LOCAL blocks in the conditional-independence graph of the joint stationary distribution. This places $\sigma^2_C$ within the broader theory of partial-correlation graphs and Markov properties for multivariate Gaussian distributions. In a complementary direction, $\sigma^2_C$ admits an information-theoretic interpretation as the conditional-information loss from the model misspecification that imposes block-diagonality; the connection to White's framework \cite{White1982} for likelihood-based misspecification is direct under joint Gaussianity, where $\sigma^2_C$ corresponds to a specific quasi-KL-divergence component. In time-series econometrics, $\sigma^2_C$ complements existing forecast-error decomposition literature \cite[Ch.~2.2.2, Ch.~3.5]{Lutkepohl2005} by providing the closed-form differential at the partition level rather than the within-block forecast-error covariance per se.

\paragraph{Scope and companion work.} The framework as presented is restricted to stationary VAR(1) processes with a given LOCAL/REMAINDER partition. The VAR$(p)$ extension, of practical interest for most macroeconomic and time-series applications, can be approached via companion-form embedding; the conditional-independence interpretation of Corollary~\ref{cor:cond-indep} may not survive intact in the companion form because the augmented state space mixes contemporaneous and lagged components, and the graphical-separation structure at the augmented level is a different geometric object from the per-variable conditional-independence graph at the original level. We leave the VAR$(p)$ generalization, and the precise statement of when the cluster-decomposition's CI interpretation does or does not lift, open. Companion empirical work applying the $\sigma^2_C$ framework to heterogeneous macroeconomic panels appears separately.

\paragraph{Open questions.} Beyond VAR$(p)$, several extensions remain open: (i)~closed-form treatment of regularization mechanisms (matrix-Ridge, shrinkage) in partitioned VAR forecasting, where $\sigma^2_C$ captures only the unregularized portion of the joint-vs-block-diagonal differential; (ii)~the $\sigma^2_C$ structure under non-Gaussian innovations, where the Schur-complement interpretation persists at the second-moment level but the conditional-independence interpretation in Corollary~\ref{cor:cond-indep} requires care; (iii)~unrestricted symmetric $A$ in Proposition~\ref{prop:P3}, where general $\Sigma_\varepsilon$ does not admit a clean closed form; (iv)~Edgeworth correction for the small-$T$ size of the Wald test in Theorem~\ref{thm:T5}; (v)~connections to spectral forecast-error decompositions in network time-series.
...
\section{Limiting cases for VAR$(1)$: statements and proofs}\label{app:C}

This appendix presents five named structural regimes under which $\sigma^2_C$ admits a closed-form simplification or bound, with statements and proofs combined. All results specialize Theorem~\ref{thm:T2} and assume (A1)--(A3) of the VAR$(1)$ baseline.

\begin{proposition}[Block-diagonal $A$]\label{prop:P1}
If $A_{\REM, \LOC_j} = \mathbf{0}$ for all $j$, then $\sigma^2_C = 0$.
\end{proposition}

This is the ``no cross-block coupling'' baseline and supplies the null hypothesis tested in \S\ref{sec:test} (Theorem~\ref{thm:T5}). The converse holds unconditionally: by the partitioned-inverse identity, $\Mloc^{-1} = (\Sigmaf^{-1})_{\LOC, \LOC} \succ 0$ since $\Sigmaf \succ 0$ under (A1)+(A2), and the bilinear form $\sigC = \Ntot^{-1} \trace[A_{\REM, \LOC} \Mloc A_{\REM, \LOC}^\top]$ vanishes iff $A_{\REM, \LOC} = \mathbf{0}$.

\paragraph{Proof of Proposition~\ref{prop:P1}.} By Theorem~\ref{thm:T2} (T2a), $\sigma^2_{C, \LOC_j \leftrightarrow \REM} = 0$ for each $j$ since $A_{\REM, \LOC_j} = \mathbf{0}$. Each off-diagonal cross term in $\sigma^2_{C, \mathrm{cross}}$ likewise vanishes. $\square$
...
\end{equation*}
where $\Mloc$ is the LOC$\times$LOC Schur complement. Under joint Gaussianity, $v^\top \Mloc v = \Var(v^\top f_\LOC \mid f_\REM)$.
\end{proposition}

The rank-one form factorizes the cross-block dependence into a single REM-direction $u$ and a single LOC-direction $v$; the regime is natural when REMAINDER is ``downstream'' of a single latent signal extracted from LOCAL.

\paragraph{Proof of Proposition~\ref{prop:P2}.} Partition $v = (v_1, \ldots, v_J)^\top$ conformably with the LOCAL partition, so $A_{\REM, \LOC_j} = u v_j^\top$. For diagonal contributions (T2a),
\begin{equation*}
...
\begin{proposition}[Symmetric $A$, isotropic $\Sigmae$]\label{prop:P3}
Suppose $A = A^\top$ and $\Sigmae = \sigma_\varepsilon^2 I_K$. Then $\Sigmaf = \sigma_\varepsilon^2 (I_K - A^2)^{-1}$, and in the eigenbasis $A = U \Lambda U^\top$ with $P_\REM := U^\top E_\REM$,
\begin{multline*}
\sigma^2_C = \frac{\sigma_\varepsilon^2}{\Ntot} \Bigl\{ \trace[P_\REM^\top \Lambda^2 (I - \Lambda^2)^{-1} P_\REM] \\
- \trace[P_\REM^\top \Lambda (I - \Lambda^2)^{-1} P_\REM (P_\REM^\top (I - \Lambda^2)^{-1} P_\REM)^{-1} P_\REM^\top (I - \Lambda^2)^{-1} \Lambda P_\REM] \Bigr\}.
\end{multline*}
\end{proposition}

The closed form requires $\Sigmae$ to commute with $A$; the isotropic case is the simplest sub-case. Modes with $|\lambda_k|$ near $1$ and large REM-projection contribute most strongly. General $\Sigmae$ does not admit a clean spectral form (open question, \S\ref{sec:disc}).

\paragraph{Proof of Proposition~\ref{prop:P3}.} For (\ref{eq:VAR1}) with $A = A^\top$ and $\Sigmae = \sigma_\varepsilon^2 I$, the discrete Lyapunov equation reduces to $\Sigmaf = A \Sigmaf A + \sigma_\varepsilon^2 I$. By $\rho(A^2) < 1$, the Neumann-series ansatz $\Sigmaf = \sigma_\varepsilon^2 \sum_{n \ge 0} A^{2n}$ converges and satisfies the equation:
\begin{equation*}
...
Suppose $(\Sigmaf)_\REM = \sigma^2 (I_n + \rho J_n)$ with $n = K_\REM$, $J_n = \mathbf{1}_n \mathbf{1}_n^\top$, $\sigma^2 > 0$, $\rho \in (-1/(n-1), 1)$. Then by Sherman-Morrison, $(\Sigmaf)_\REM^{-1} = \sigma^{-2} (I_n - \rho/(1 + n\rho) \cdot J_n)$, and
\begin{equation*}
M(\LOC_j, \LOC_j) = (\Sigmaf)_{\LOC_j, \LOC_j} - \sigma^{-2} (\Sigmaf)_{\LOC_j, \REM} (\Sigmaf)_{\REM, \LOC_j} + \frac{\rho}{\sigma^2 (1 + n\rho)} r_j r_j^\top,
\end{equation*}
where $r_j := (\Sigmaf)_{\LOC_j, \REM} \mathbf{1}_n$. Substituting into (\ref{eq:T2}) gives explicit $\sigma^2_C$ in $(\sigma^2, \rho, A_{\REM, \LOC}, (\Sigmaf)_\LOC)$.
\end{proposition}

The equicorrelated structure separates $(\Sigmaf)_\REM^{-1}$ into an isotropic part plus a rank-one correction; the latter manifests as the $r_j r_j^\top$ term in $M(\LOC_j, \LOC_j)$.

\paragraph{Proof of Proposition~\ref{prop:P4}.} The Sherman-Morrison identity gives
\begin{equation*}
...
The bracketed factor is the ``amplification factor'' from the LOCAL conditional-covariance structure given REM. Tightness depends on alignment of $A_{\REM, \LOC_j}$ with top eigendirections of $M(\LOC_j, \LOC_j)$.

\paragraph{Proof of Proposition~\ref{prop:P5}.} For the diagonal contributions, the trace inequality $\trace(B^\top M B) \le \lambda_{\max}(M) \cdot \|B\|_F^2$ for symmetric PSD $M$ gives
\begin{equation*}
...
\paragraph{Vech-vs-vec convention for $V$.} Equation (\ref{eq:gradSigma}) gives the unconstrained matrix-derivative of $\varphi$ at the symmetric $\Sigmaf$. To pair with the vech-form $\Omega_{ff}$ block in $V = (\nabla \varphi)^\top \Omega (\nabla \varphi)$, the corresponding vech-gradient is
\[
\partial \varphi / \partial \vech(\Sigmaf) = D_K^\top \mathrm{vec}(\partial \varphi / \partial \Sigmaf),
\]
where $D_K$ is the $K^2 \times K(K+1)/2$ duplication matrix \cite[Appendix~A.12]{Lutkepohl2005}. Equivalently, one may work in vec form throughout, with $\Omega_{ff}^{\mathrm{vec}} = D_K \Omega_{ff} D_K^\top$ and the symmetric matrix-gradient (\ref{eq:gradSigma}) vectorised. The reproduction code (\path{verify/grad_sigma.py}, \path{verify/sampling.py}) uses the vec convention consistently and recovers $V$ to machine precision against the finite-difference benchmark.
...
\end{equation*}
where $R(h) := \E[f_t f_{t+h}^\top]$ is the autocovariance, $D_K^+$ the Moore-Penrose pseudoinverse of the duplication matrix $D_K$, and $K_{KK}$ the $K^2 \times K^2$ commutation matrix (Magnus-Neudecker conventions; see L\"utkepohl \cite[Appendix~A.12]{Lutkepohl2005}). Under VAR(1), $R(h) = \Sigmaf (A^\top)^h$ for $h \ge 0$ and $R(h) = A^{|h|} \Sigmaf$ for $h < 0$. Using $\Sigmaf (A^\top)^h \otimes \Sigmaf (A^\top)^h = (\Sigmaf \otimes \Sigmaf)(A^\top \otimes A^\top)^h$ together with the geometric-series identity $\sum_{h \ge 0} M^h = (I - M)^{-1}$ for $M = A^\top \otimes A^\top$ (spectral radius $\rho(M) = \rho(A)^2 < 1$ under (A1)), the long-run sum admits the closed form
\begin{equation*}
\sum_{h \ge 0} (\Sigmaf (A^\top)^h) \otimes (\Sigmaf (A^\top)^h) = (\Sigmaf \otimes \Sigmaf) \big[I_{K^2} - A^\top \otimes A^\top\big]^{-1}.
\end{equation*}
The $h < 0$ summand is symmetric and yields the transpose of the $h > 0$ contribution.

\paragraph{$\Omega_{Af}$ block.} The cross-covariance of $\sqrt{T}(\Ehat_T - A)$ and $\sqrt{T}(\widehat{\Sigma}_{f, T} - \Sigmaf)$ requires a long-run covariance of mixed-order moments of $f_t$ and $\varepsilon_t$. We do not give a closed form here. In practice $\widehat{\Omega}_{Af}$ is estimated either by Newey-West HAC on the joint moment process \cite{NeweyWest1987}, by stationary bootstrap \cite{PolitisRomano1994}, or directly by Monte Carlo on a long auxiliary panel under the fitted $(\Ehat_T, \widehat{\Sigma}_{f, T})$; all three are consistent under (A1)--(A4) and do not require joint Gaussianity of $\varepsilon_t$. The reproduction code (\path{verify/sampling.py}) uses the Monte-Carlo route at $T_{\mathrm{aux}} = 10000$ and recovers $V$ within $2.1\%$ of $V_{\mathrm{analytic}}$ at $T = 10000$ (\S\ref{sec:sim}).
...
% ════════════════════════════════════════════════════════════════════════════
\section{Extension to VAR$(p)$: derivations and numerical verification}\label{app:VARp}
% ════════════════════════════════════════════════════════════════════════════

This appendix presents the derivations supporting \S\ref{sec:varp} and summarizes the numerical-verification battery (scripts \path{verify/var_p/E1--E12}).

\subsection{Proof of Theorem~\ref{thm:T1p} (closed form)}

The four-step proof of Theorem~\ref{thm:T1} extends verbatim. Step~1: joint forecast-error variance at REM is $(\Sigmae)_\REM$ under joint VAR$(p)$ stationarity. Step~2: by the BLP identity applied to $f_{\REM, t}$ given $Z_t := (f_{\REM, t-1}, \ldots, f_{\REM, t-p})$, the restricted forecast-error variance is $(\Sigmaf)_\REM - C \Gamma^{-1} C^\top$ with $C, \Gamma$ defined in \S\ref{sec:varp}. Step~3: the difference is $(\Sigmaf)_\REM - C \Gamma^{-1} C^\top - (\Sigmae)_\REM$. Step~4: the VAR$(p)$ Yule-Walker block identity $(\Sigmae)_\REM = (\Sigmaf)_\REM - \sum_{h=1}^p (A_h R(h)^\top)_\REM$ (obtained by taking the REM block of $R(0) = \sum_h A_h R(h)^\top + \Sigmae$) substitutes, cancelling $(\Sigmaf)_\REM$, and gives (\ref{eq:T1p}). $\square$

\subsection{Companion-form representation}

Let $F_t = (f_t, f_{t-1}, \ldots, f_{t-p+1})^\top \in \mathbb{R}^{Kp}$. Then $F_t = \widetilde A F_{t-1} + B \varepsilon_t$ with $B = (I_K, \mathbf{0}, \ldots, \mathbf{0})^\top \in \mathbb{R}^{Kp \times K}$ and companion-form transition $\widetilde A \in \mathbb{R}^{Kp \times Kp}$. The companion-form stationary covariance $\Sigma_F$ solves $\Sigma_F = \widetilde A \Sigma_F \widetilde A^\top + B \Sigmae B^\top$, whose $(0, h)$ blocks (for $h = 0, \ldots, p-1$) yield $R(h)$ and whose lifted REMAINDER sub-block $(\Sigma_F)_{\REM_\text{comp}}$ equals the block-Toeplitz $\Gamma$. Applying Theorem~\ref{thm:T1} to $(\widetilde A, \Sigma_F)$ at the partition $\REM_\text{comp}$ produces a $K_\REM p \times K_\REM p$ Schur complement whose trace equals $\Ntot \cdot \sigma^{2,(p)}_C$. The lifted innovation covariance $B \Sigmae B^\top$ is rank-$K$ (singular in dim $Kp$), so (A2) fails at the lifted system; this does not affect Theorem~\ref{thm:T1}'s Schur form since the proof uses only $(\Sigma_F)_{\REM_\text{comp}} \succ 0$ (automatic from $\Gamma \succ 0$). Furthermore, the off-diagonal blocks of the Schur complement at the lifted partition vanish at the level of the (rest err)$-$(joint err) difference: the lagged-REM components of $F_t$ are observed in $F_{t-1}$, so their forecast errors are zero in both the joint and restricted cases. Empirically, the off-diagonal entries are machine zero ($< 10^{-14}$ in all tested regimes).

\subsection{Theorem~\ref{thm:T2p} and Corollary~\ref{cor:cond-indep-p}}

Theorem~\ref{thm:T2}'s bilinear expansion lifts verbatim at the companion-state level: expanding $\widetilde A_{\REM_\text{comp}, \cdot} F_{t-1}$ over the lifted LOCAL$_{j, \text{comp}}$ blocks plus $\REM_\text{comp}$ produces
\begin{equation}
\Ntot \cdot \sigma^{2,(p)}_C = \sum_{a, b} \trace[\widetilde A_{\REM_\text{comp}, a} \widetilde M^\text{comp}(a, b) \widetilde A_{\REM_\text{comp}, b}^\top],
\end{equation}
where the sum is over $a, b \in \{\LOC_{1, \text{comp}}, \ldots, \LOC_{J, \text{comp}}, \REM_\text{comp}\}$ and $\widetilde M^\text{comp}(\REM_\text{comp}, \cdot) = \mathbf{0}$ by the same Schur structural identity as before. Restricting to $a, b \in \{\LOC_{j, \text{comp}}\}$ and splitting diagonal $(j = k)$ vs off-diagonal $(j \ne k)$ yields (\ref{eq:T2p}). For Corollary~\ref{cor:cond-indep-p}, the structural-iff argument is identical to Corollary~\ref{cor:cond-indep}'s proof in Appendix~\ref{app:B}, applied to $\widetilde M^\text{comp}$ at the $(Kp)$-dim companion state. Lauritzen \cite[\S 5.1.3]{Lauritzen1996} gives the Schur-complement$\leftrightarrow$CI equivalence at the Gaussian level.

\subsection{Sampling theory at VAR$(p)$}

The plug-in estimator $\widehat\sigma^{2,(p)}_C := \varphi^{(p)}(\widehat A_{1, T}, \ldots, \widehat A_{p, T}, \widehat \Sigmaf{}_{,T})$ uses OLS-VAR$(p)$ coefficient estimates $\widehat A_{1, T}, \ldots, \widehat A_{p, T}$ and the sample second-moment $\widehat\Sigmaf{}_{,T} = T^{-1} \sum_t f_t f_t^\top$. The smooth functional $\varphi^{(p)}$ inherits continuity on the open set where $\Gamma \succ 0$ from the block-Toeplitz inverse; continuous-mapping gives consistency. Asymptotic normality follows from delta method on $\varphi^{(p)}$ with the joint asymptotic covariance $\Omega^{(p)}$ of $(\mathrm{vec}\,\widehat A_{1, T}, \ldots, \mathrm{vec}\,\widehat A_{p, T}, \vech\widehat\Sigmaf{}_{,T})$ given in L\"utkepohl \cite[Prop.~3.2, Ch.~3.4]{Lutkepohl2005}. The gradient $\nabla \varphi^{(p)}$ has dimension $p K^2 + K(K+1)/2$ and is computed via finite differences in practice or via the chain rule on the block-Toeplitz $\Gamma^{-1}$ derivative.

For Theorem~\ref{thm:T5p}, L\"utkepohl Prop.~3.1 gives $\sqrt{T} \cdot \vec(\widehat A_{1:p, T} - A_{1:p}) \xrightarrow{d} \mathcal{N}(\mathbf{0}, \Gamma_p^{-1} \otimes \Sigmae)$ where $\widehat A_{1:p, T} := (\widehat A_{1, T}, \ldots, \widehat A_{p, T})$ has columns indexed by $Kp$-dim regressors. By the Kronecker structure, the asymptotic covariance of $\eta_T = \vec(\widehat A_{1:p, \REM, \LOC, T})$ is $(\Gamma_p^{-1})_{\LOC \times \text{all lags},\,\LOC \times \text{all lags}} \otimes (\Sigmae)_\REM = (M_\LOC^{(p)})^{-1} \otimes (\Sigmae)_\REM$. The inverse, used in (\ref{eq:T5p}), is $M_\LOC^{(p)} \otimes (\Sigmae)_\REM^{-1}$, which yields the $\chi^2$ limit. For $p = 1$, $\Gamma_p = \Sigmaf$ and $M_\LOC^{(1)} = \Mloc$, recovering Theorem~\ref{thm:T5}.

Near-degeneracy: by the same two-factor $\Gamma^{-1}$ argument as in Appendix~\ref{app:F}, $\partial \varphi^{(p)} / \partial \Sigmaf \sim 1/\lambda_{\min}(\Gamma)^2$ in the dominant direction, hence $V^{(p)} \sim 1/\lambda_{\min}(\Gamma)^4$.

\subsection{Numerical verification battery (E1--E12)}\label{app:VARp_tables}

Reproduction scripts: \path{verify/var_p/E1_thm1_var_p.py} through \path{E12_large_K.py}. All output as JSON in the same directory.

\begin{center}
\small
\begin{tabular}{lll}
\toprule
Experiment & Verdict & Key numerical evidence \\
\midrule
E1 closed-form at $p=2$, $K \in \{4, 6\}$            & PASS  & V1 residual $\le 10^{-15}$; V2 MC rel.\ err.\ 0.14--1.11\% \\
E2 companion-form equiv at $p \in \{2, 3\}$          & PASS  & max residual $5 \times 10^{-16}$; off-block max $1.1 \times 10^{-14}$ \\
E3 CI interpretation at $p=2$                        & PARTIAL & cluster decomp lifts exactly; contemp.\ CI does not lift \\
E4 closed-form sweep $p \in \{2,3,5,7,10\}$, $K$ up to $12$ & PASS  & 25/25 cells; max V1, V3 residuals $9 \times 10^{-15}$; 9/9 MC PASS \\
E5 companion-form sweep $p$ up to $10$, $K$ up to $12$ & PASS  & 25/25 cells; max residual $9 \times 10^{-15}$ \\
E6 CI subclass battery (6 subclasses $\times$ 10 seeds) & PARTIAL & contemporaneous CI insufficient; rel cross 1--14\% across DGPs \\
E7 sampling theory at $p=2$, $T \in \{500, \ldots, 20000\}$ & PASS & rel err 0.014\% at $T=20000$; $\sqrt{T}\!\cdot\!\mathrm{SD}$ ratio $1.135$; CI95 cov 96.0\% \\
E8 bias at $p=2$, $n_{\mathrm{rep}} = 3000$         & PASS  & $T \cdot$ bias at $T=500$: $+5.66 \pm 2.26$; rel bias $0.33\%$ \\
E9 Wald test (Theorem~\ref{thm:T5p}) at $p=2$       & PASS  & emp.\ size $0.0525$ ($T=1000$), $0.0450$ ($T=2000$); power $96\%$ at $\sigC = 0.064$ \\
E10v2 near-degeneracy $V \sim 1/\lambda_{\min}(\Gamma)^4$ & PASS  & log-log slope $\mathbf{-4.0000}$ (theoretical $-4$) \\
E11 Sympy exact at $K=3$, $p=2$                     & PASS  & closed-form $\equiv$ BLP $\equiv$ companion (identical rationals) \\
E12 large-$K$ ($K \in \{12, 20, 30\}$, $p \in \{2, 3\}$) & PASS  & sub-ms closed form at $K=30$; sub-second OLS plug-in at $T=10000$ \\
\bottomrule
\end{tabular}
\end{center}

\smallskip
\noindent\refstepcounter{table}\label{tab:varp_E_summary}\textbf{Table~\ref{tab:varp_E_summary}.} VAR$(p)$ verification battery summary.
\smallskip

\paragraph{Remark on E6 / Contemporaneous CI failure.} E6 tested six structural subclasses with progressively stronger restrictions: (S1) baseline; (S2) no LOC--LOC at any lag + block-diagonal $\Sigmae$; (S3) S2 + AR$(1)$-within-REM dynamics ($A_h$ has zeros at $(\REM, \REM)$ for $h \ge 2$); (S4) S3 + $A_h$ has zeros at $(\REM, \LOC)$ for $h \ge 2$; (S5) S3 + $A_h$ has zeros at $(\LOC, \REM)$ for $h \ge 2$; (S6) S3 + S4 + S5 combined (REM appears only at lag 1 in every block). At $p = 2$, S6 mean relative cross over 10 seeds is $3.4\%$ (max $14\%$). At $p = 3$, S6 mean is $0.53\%$ (max $\sim 1\%$). Even maximal structural restriction does not drive $\sigma^{2,\text{comp}}_{C, \mathrm{cross}}$ to zero; finite $p$-lag conditioning on REM leaks dependence from REM-history past the conditioning window. The $(Kp)$-dimensional CI graph of $F_t$ remains the load-bearing object.

\paragraph{Computational scale (E12).} Closed-form $\sigma^{2,(p)}_C$ at $K = 30$, $p = 2$ computes in $0.17$~ms; the companion-form Lyapunov solve scales as $O((Kp)^3)$ via standard \texttt{scipy.linalg.solve}. OLS-VAR$(p)$ at $K = 30$, $T = 10000$, $p = 2$ completes in $\sim 0.16$~s on a single laptop CPU. The framework is practical at applied multivariate-panel sizes.
...
\bibitem{NeweyWest1987} W.~K. Newey and K.~D. West. \emph{A simple, positive semi-definite, heteroskedasticity and autocorrelation consistent covariance matrix}. Econometrica, 55(3):703--708, 1987.
...
\bibitem{PolitisRomano1994} D.~N. Politis and J.~P. Romano. \emph{The stationary bootstrap}. Journal of the American Statistical Association, 89(428):1303--1313, 1994.
